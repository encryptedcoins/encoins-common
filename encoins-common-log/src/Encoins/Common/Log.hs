...
module Encoins.Common.Log where
...
Your config has to have three extra fields environment, verbosity, severity

withIpfsEnv :: (Env -> IO ()) -> IO ()
withIpfsEnv action = do
  config <- getConfig
  let logEnv = mkLogEnvCustom
        "Custom name"
        (environment config)
        (verbosity config)
        (severity config)
  withLogEnv logEnv $ \le -> do
    let env = ... make 'env' with 'le'
    action env
...
mkLogEnv :: Namespace
  ...
  -> Severity
  -> IO LogEnv
mkLogEnv name katipEnv verbosity severity = do
  handleScribe <- mkHandleScribeWithFormatter
    ...
    (permitItem severity)
    verbosity
  initiatedLogEnv <- initLogEnv name katipEnv
  registerScribe
    "stdout"
    handleScribe
